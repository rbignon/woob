...
class AppPaste(ReplApplication):
    ...
    def _get_op(self, _id, binary, command="get"):
        ...
        if binary:
            if self.interactive:
                if not self.ask(
                    "The console may become messed up. Are you sure you want to show a binary file on your terminal?",
                    default=False,
                ):
    ...
    def str_to_duration(self, s):
        ...
        argsmap = {
            "Y|y|year|years|yr|yrs": 365.25 * 24 * 3600,
    ...
    def add_application_options(self, group):
        group.add_option("-p", "--public", action="store_true", help="Make paste public.")
        group.add_option("-t", "--title", action="store", help="Paste title", type="string")
        group.add_option(
            "-m",
            "--max-age",
            action="store",
        ...
        group.add_option("-E", "--encoding", action="store", help="Input encoding", type="string")
