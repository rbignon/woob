...
from woob.capabilities.housing import ADVERT_TYPES, HOUSE_TYPES, POSTS_TYPES, CapHousing, Query
...
class AppHousing(ReplApplication):
    ...
    CONFIG = {"queries": {}}
    EXTRA_FORMATTERS = {
        "housing_list": HousingListFormatter,
        "housing": HousingFormatter,
    }
    COMMANDS_FORMATTERS = {"search": "housing_list", "info": "housing", "load": "housing_list"}
    ...
    def do_search(self, line):
        ...
        while pattern:
            ...
            while r != "":
                for i, city in enumerate(cities):
                    print(
                        "  %s%2d)%s [%s] %s (%s)"
                        % (self.BOLD, i + 1, self.NC, "x" if city in query.cities else " ", city.name, city.backend)
                    )
        ...
        while r != "":
            for i, good in enumerate(HOUSE_TYPES, 1):
                print("  %s%2d)%s [%s] %s" % (self.BOLD, i, self.NC, "x" if good in query.house_types else " ", good))
        ...
        while _type not in range(len(posts_types)):
            for i, t in enumerate(posts_types):
                print("  %s%2d)%s %s" % (self.BOLD, i, self.NC, t))
        ...
        while r != "":
            for i, good in enumerate(ADVERT_TYPES, 1):
                print("  %s%2d)%s [%s] %s" % (self.BOLD, i, self.NC, "x" if good in query.advert_types else " ", good))
