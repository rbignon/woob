...
class RedmineBrowser(LoginBrowser):
    ...
    issues = URL(r"/projects/[\w-]+/issues$", r"/issues/?\?.*$", r"/issues$", IssuesPage)
    ...
    def query_issues(self, project_name, **kwargs):
        ...
        data = (
            (self.METHODS[method]["project_id"], project_name),
        ...
        return {
            "project": self.page.get_project(project_name),
    ...
    def create_category(self, project, name, token):
        ...
        m = re.search(r"""value=\\\\"(\d+)\\\\" selected""", r)
        if m:
            return m.group(1)
