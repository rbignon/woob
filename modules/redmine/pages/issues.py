    ...
    def parse_datetime(self, text):
        ...
        if m:
            date = datetime.datetime(
                int(m.group(3)), int(m.group(1)), int(m.group(2)), int(m.group(4)), int(m.group(5))
            )
            if m.group(6) == "pm":
    ...
    def iter_issues(self):
        ...
        for tr in issues.iter("tr"):
            ...
            for td in tr.iter("td"):
                ...
                a = td.find("a")
                if a is not None:
                    if a.attrib["href"].startswith("/users/") or a.attrib["href"].startswith("/versions/"):
                ...
                if field.endswith("_on"):
                    ...
                    if m:
                        text = datetime.datetime(int(m.group(1)), int(m.group(2)), int(m.group(3)))
...
class NewIssuePage(BaseIssuePage):
    ...
    def iter_custom_fields(self):
        for div in self.doc.xpath(
            '//form//input[starts-with(@id, "issue_custom_field")]|//form//select[starts-with(@id, "issue_custom_field")]'
        ):
...
class IssuePage(NewIssuePage):
    ...
    def get_params(self):
        ...
        if "title" not in alist[i].attrib:
            params["author"] = (int(alist[i].attrib["href"].split("/")[-1]), to_unicode(alist[i].text))
            i += 1
        else:
        ...
        for div in content.xpath('.//div[has-class("journal")]'):
            ...
            if len(user_link) > 0:
                update["author"] = (int(user_link[0].attrib["href"].split("/")[-1]), to_unicode(user_link[0].text))
            else:
