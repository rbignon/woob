...
class CreditMutuelBrowser(TwoFactorBrowser):
    ...
    loans_insurance = URL(r"/(?P<subbank>.*)fr/assurances/consultation/ASSEMPR.aspx", LoansInsurancePage)
    coming = URL(r"/(?P<subbank>.*)fr/banque/mvts_instance.cgi.*", ComingPage)
    info = URL(r"/(?P<subbank>.*)fr/banque/BAD.*", EmptyPage)
    change_pass = URL(
        r"/(?P<subbank>.*)fr/validation/change_password.cgi", "/fr/services/change_password.html", ChangePasswordPage
    )
    verify_pass = URL(
    ...
    new_home = URL(
        r"/(?P<subbank>.*)fr/banque/pageaccueil.html", r"/(?P<subbank>.*)banque/welcome_pack.html", NewHomePage
    )
    empty = URL(
    ...
    li_history = URL(
        r"/(?P<subbank>.*)fr/assurances/SYNASSVIE.aspx\?_tabi=C&_pid=ValueStep&_fid=GoOnglets&Id=3", LIAccountsPage
    )
    ...
    cards_list = URL(
        r"/(?P<subbank>.*)fr/banque/pro/ENC_liste_ctr.*", r"/(?P<subbank>.*)fr/banque/pro/ENC_detail_ctr", CardsListPage
    )
    ...
    information_confirmation_page = URL(
        r"/(?P<subbank>.*)fr/client/paci_engine/information-client.html", InformationConfirmationPage
    )
    ...
    def get_expire(self):
        ...
        if self.twofa_auth_state and self.twofa_auth_state.get("expires"):
            expires = (
                datetime.fromtimestamp(self.twofa_auth_state["expires"], tz.tzlocal())
    ...
    def finalize_twofa(self, twofa_data):
        ...
        final_location(twofa_data["final_url"], data=twofa_data["final_url_params"], allow_redirects=False)
    ...
    def check_redirections(self):
        ...
        if "validation.aspx" in location and not self.is_interactive and datetime.now() > twofa_limit_date:
    ...
    def check_auth_methods(self):
        ...
        if self.mobile_confirmation.is_here():
            ...
            if self.page.is_waiting_for_sca_activation():
                raise ActionNeeded(
                    locale="fr-FR",
                    message="Une intervention de votre part est requise sur votre espace client.",
    ...
    def init_login(self):
        ...
        if self.solidarity.is_here():
            ...
            raise ActionNeeded(
                locale="fr-FR",
                message="Un message relatif au don pour les personnes en situation d'handicap est disponible sur votre espace.",
    ...
    def get_accounts_list(self):
        if not self.accounts_list:
            ...
            companies = (
                self.page.companies_link()
                if self.cards_activity.is_here()
                else [self.page] if self.is_new_website else []
            )
            ...
            for acc in self.accounts_list:
                ...
                if acc.type == Account.TYPE_CARD and not empty(getattr(acc, "_parent_id", None)):
                    ...
                    self.logger.warning('There is an untyped account: please add "%s" to ACCOUNT_TYPES.', acc.label)
    ...
    def get_current_sub_bank(self, *, force=False):
        ...
        if self.currentSubBank and self.currentSubBank.startswith("banqueprivee"):
    ...
    def list_operations(self, page, account):
        ...
        if account.type == Account.TYPE_SAVINGS and re.match("Capital Expansion|Plan Epargne Logement", account.label):
    ...
    def get_history(self, account):
        ...
        if len(account.id) >= 16 and account.id[:16] in self.cards_histo_available:
                        ...
                        for i in range(1, 100):
                            ...
                            if "fid=GoMonth&mois=" in self.url:
                                ...
                                if m:
                                    m = m.group(1)
                                self.location(
                                    "CRP8_SCIM_DEPCAR.aspx?_tabi=C&a__itaret=as=SCIM_ListeActivityStep%3a%3a%2fSCIM_ListeRouter%3a%3a&a__mncret=SCIM_LST&a__ecpid=EID2011&_stack=_remote::moiSelectionner={},moiAfficher={},typeDepense=T&_pid=SCIM_DEPCAR_Details".format(
                                        m, half_history
                                    ),
                                    data=data,
                                )
            ...
            deferred_date = None
            cards = (
                [page.select_card(account.number) for page in account._card_pages]
    ...
    def check_and_initiate_transfer_otp(self, transfer, account=None, recipient=None):
        ...
        if self.page.needs_otp_validation():
            ...
            raise TransferStep(
                transfer, Value("code", label="Veuillez saisir le code reçu par sms pour confirmer votre opération")
            )
    ...
    def execute_transfer(self, transfer, **params):
        ...
        if self.page.doc.xpath('//form[@id="P:F"]//input[@type="submit" and contains(@value, "Confirmer")]'):
    ...
    def iter_documents_for_account(self, account_id, account_label):
        ...
        access_not_allowed_msg = (
            "Vous ne disposez pas des droits nécessaires pour accéder à cette partie de l'application."
        )
