    ...
    class Item(MyRecipient):
        ...
        def parse(self, el):
            ...
            if category == "Interne":
                ...
                match = [acc for acc in accounts if acc.id == long_id and acc.type != Account.TYPE_CARD]
                if not match:
                    ...
                    match = [acc for acc in accounts if short_id in acc.id and acc.type != Account.TYPE_CARD]

                # Not all internal accounts are returned by get_accounts_list
                if not match:
                    self.logger.warning(
                        "skipping internal recipient without a matching account: %r (%r)", short_id, long_id
                    )
                    raise SkipItem()
                ...
                self.env["bank_name"] = "Caisse d'Épargne"
...
class ProAddRecipientOtpPage(IndexPage):
    ...
    def is_here(self):
        return self.need_auth() and self.doc.xpath(
            '//span[@id="MM_ANR_WS_AUTHENT_ANR_WS_AUTHENT_SAISIE_lblProcedure1"]'
        )
...
class ProTransferSummaryPage(TransferErrorPage, IndexPage):
    ...
    def populate_reference(self, transfer):
        transfer.id = Regexp(
            CleanText('//span[@id="MM_VIREMENT_AR_VIREMENT_lblVirementEnregistre"]'), r"(\d+( - \d+)?)"
        )(self.doc)
        return transfer
...
class TransferConfirmPage(TransferErrorPage, IndexPage):
    ...
    def update_transfer(self, transfer, account=None, recipient=None):
        ...
        transfer.exec_date = Date(
            CleanText('.//tr[th[contains(text(), "En date du")]]/td[not(@class)]'), dayfirst=True
        )(self.doc)
        ...
        if recipient:
            ...
            if recipient.category == "Externe":
                all_text = Upper(CleanText(""".//tr[th[contains(text(), "Compte à créditer")]]/td[not(@class)]"""))(
                    self.doc
                )
