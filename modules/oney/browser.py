...
from .pages import (
    ...
    OtherDashboardPage,
    OtherOperationsPage,
    SendCompleteStepPage,
    SendInitStepPage,
    SendRiskEvaluationPage,
    SendUsernamePage,
)
...
class OneyBrowser(TwoFactorBrowser):
            ...
            if step_action == "init":
                if step_type in ("SCA_PUSH", "PHONE_OTP"):
                    ...
                    if step_type == "PHONE_OTP":
                        ...
                        label = (
                            "Un nouveau code de sécurité vous a été envoyé par SMS au %s." % extra_data["masked_phone"]
                        )
    ...
    def other_space_params_headers(self):
        return {"Header.%s" % key: value for key, value in self.params_headers.items()}
    ...
    def check_auth_error(self):
        ...
        if error:
            ...
            browser_unavailable_re = (
                r"Authenticator : Invalid CA response code : 504 Gateway Timeout" + r"|.TechnicalError. Read timed out"
            )
            ...
            if "NOT_ACTIVATED" in error:
                ...
                raise ActionNeeded(
                    locale="fr-FR",
                    message="Une validation par e-mail est nécessaire pour activer votre compte.",
                )
