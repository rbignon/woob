    ...
    class iter_transactions(ListElement):
        ...
        def next_page(self):
            options = self.page.doc.xpath(
                '//select[@id="periode"]//option[@selected="selected"]/preceding-sibling::option[1]'
            )
            if options:
                data = {
        ...
        class item(ItemElement):
            ...
            def obj_coming(self):
                ...
                if cur_type == Account.TYPE_LOAN:
                    return CleanDecimal.SI(Dict("depreciableAccount/installments/0/totalAmount", default=0), sign="-")(
                        self
                    )
