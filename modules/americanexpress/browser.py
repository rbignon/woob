...
class AmericanExpressBrowser(TwoFactorBrowser):
    ...
    fingerprint = URL(
        r"https://www.cdn-path.com/cc.js\?=&sid=ee490b8fb9a4d570&tid=(?P<transaction_id>.*)&namespace=inauth",
        FingerprintPage,
    )
    ...
    json_balances2 = URL(
        r"/api/servicing/v1/financials/transaction_summary\?type=split_by_cardmember&statement_end_date=(?P<date>[\d-]+)",
        JsonBalances2,
    )
    json_pending = URL(
        r"/api/servicing/v1/financials/transactions\?limit=1000&offset=(?P<offset>\d+)&status=pending", JsonHistory
    )
    json_posted = URL(
    ...
    currency_page = URL(
        r"https://www.aexp-static.com/cdaas/axp-app/modules/axp-balance-summary/4.7.0/(?P<locale>\w\w-\w\w)/axp-balance-summary.json",
        CurrencyPage,
    )

    no_card = URL(
        r"https://www.americanexpress.com/us/content/no-card/",
        r"https://www.americanexpress.com/us/no-card/",
        NoCardPage,
    )
    ...
    def raise_otp(self):
        ...
        if not self.auth_trusted:
            self.logger.warning("We are not trusted. There could be a problem with the fingerprinting of cc.js")
    ...
    def make_otp_delivery_payload(self, methods):
        ...
        if delivery_method == "EMAIL":
            message = (
                "Veuillez entrer le code d’authentification qui vous a été envoyé à l'adresse courriel %s."
                % display_value
            )
        else:
    ...
    def iter_coming(self, account):
        ...
        if date != today:
            try:
                ...
                self.logger.warning(
                    'No pendings page to access to, got error %s and message "%s" instead.', error_code, error_message
                )
            else:
                for tr in self.page.iter_history(periods=periods):
