...
class LoginPage(JsonPage):
    ...
    def get_user_name(self):
        return Coalesce(
            ...
            Format("%s %s", Dict("firstName", default=NotAvailable), Dict("lastName", default=NotAvailable)),
        )(self.doc)
...
class AVInvestmentsPage(LoggedPage, JsonPage):
    def update_life_insurance_account(self, life_insurance):
        ...
        life_insurance.valuation_diff = CleanDecimal(Dict("situationFinanciereEpargne/mtpmvcnt"), default=NotAvailable)(
            self.doc
        )
...
class BoursePage(LoggedPage, HTMLPage):
    ...
    def get_fullhistory(self):
        ...
        form["beginDayfilter"] = (datetime.strptime(form["endDayfilter"], "%d/%m/%Y") - timedelta(days=730)).strftime(
            "%d/%m/%Y"
        )
        form.submit()
