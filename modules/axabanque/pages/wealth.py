...
class InvestmentPage(LoggedPage, HTMLPage):
    ...
    def detailed_view(self):
        return Attr('//button[contains(text(), "Vision détaillée")]', "data-module-open-link--link", default=None)(
            self.doc
        )
...
class InvestmentJsonPage(LoggedPage, JsonPage):
    ...
    class iter_investments(DictElement):
        ...
        class item(ItemElement):
            ...
            obj_vdate = Date(CleanText(Dict("savingsAmount/date", default="")), dayfirst=True, default=NotAvailable)
...
class AccountDetailsPage(LoggedPage, HTMLPage):
    ...
    def get_investment_url(self):
        return Attr('//div[contains(@data-analytics-label, "repartition_par_fond")]', "data-url", default=None)(
            self.doc
        )

    def get_iframe_url(self):
        return Attr(
            '//div[contains(@class, "iframe-quantalys")]', "data-module-iframe-quantalys--iframe-url", default=None
        )(self.doc)

    def get_pid(self):
        return Attr(
            '//div[@data-module="operations-movements"]', "data-module-operations-movements--pid", default=None
        )(self.doc)
...
class BourseAccountsPage(LoggedPage, HTMLPage):
    def get_cipher(self, account_number):
        return QueryValue(Link('.//a[contains(text(), "%s")]' % account_number), "cipher")(self.doc)
...
class NewInvestmentPage(LoggedPage, HTMLPage):
    ...
    class iter_investments(TableElement):
        ...
        class item(ItemElement):
            ...
            obj_code = IsinCode(
                Regexp(CleanText(TableCell("isin_code")), r"[A-Z]{2}[A-Z0-9]{10}", default=NotAvailable),
                default=NotAvailable,
            )
            obj_code_type = IsinType(Field("code"))
            obj_label = CleanText(TableCell("label"))
