...
class SprungeTest(BackendTest):
    ...
    def test_post(self):
        p = self.backend.new_paste(None, contents="Woob Test héhéhé")
    ...
    def test_notfound(self):
        for _id in ("ab", "http://sprunge.us/ab"):
            # html method
            p = self.backend.get_paste(_id)
    ...
    def test_can_post(self):
        assert 2 == self.backend.can_post("hello", public=False)
        assert 1 == self.backend.can_post("hello", public=False, title="hello")
        assert 1 == self.backend.can_post("hello", title="hello")
        assert 0 == self.backend.can_post("hello", public=True)
        assert 0 == self.backend.can_post("hello", public=False, max_age=3600 * 24)
