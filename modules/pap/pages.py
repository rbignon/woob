...
class HousingPage(HTMLPage):
    ...
    class iter_housings(ListElement):
        ...
        class item(ItemElement):
            ...
            def condition(self):
                ...
                if id is None:
                    return False
                return id and isNotFurnishedOk

            def parse(self, el):
                rooms_bedrooms_area = el.xpath('./div/a[@class="item-title"]/ul[has-class("item-tags")]/li')
                ...
                for item in rooms_bedrooms_area:
                    ...
                    if "chambre" in name.lower():
                        ...
                        name = "area"
                        value = CleanDecimal(Regexp(CleanText("."), r"(\d*\.*\d*) .*"))(item)
                    self.env[name] = value

            obj_id = Regexp(Link('./div/a[@class="item-title"]'), "/annonces/(.*)", default=None)
            ...
            obj_cost = CleanDecimal(
                CleanText('./div/a[@class="item-title"]/span[@class="item-price"]'),
                replace_dots=True,
                default=Decimal(0),
            )
            obj_currency = Currency('./div/a[@class="item-title"]/span[@class="item-price"]')
            ...
            obj_url = Format("http://www.pap.fr%s", Link('./div/a[@class="item-title"]'))

            def obj_photos(self):
                photos = []
                for img in XPath("./div/div/img/@src")(self):
                    if img.endswith("visuel-nophoto.png") or img.endswith("miniature-video.png"):
                        continue
                    photos.append(HousingPhoto("%s" % img))
                return photos
    ...
    class get_housing(ItemElement):
        ...
        obj_cost = CleanDecimal('//h1[@class="item-title"]/span[@class="item-price"]', replace_dots=True)
        obj_currency = Currency('//h1[@class="item-title"]/span[@class="item-price"]')
        obj_utilities = UTILITIES.UNKNOWN
        obj_area = Env("area")

        def obj_date(self):
            date = CleanText('//p[@class="item-date"]')(self).split("/")[-1].strip()
        ...
        def obj_station(self):
            return ", ".join(
                [
                    station.text
                    for station in XPath('//ul[has-class("item-transports")]//span[has-class("label")]')(self)
                ]
            )
        ...
        def obj_DPE(self):
            DPE = Attr('//div[has-class("energy-box")]//div[has-class("energy-rank")]', "class", default="")(self)
            if DPE:
                DPE = [x.replace("energy-rank-", "").upper() for x in DPE.split() if x.startswith("energy-rank-")][0]
