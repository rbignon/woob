...
class AccountsPage(LoggedPage, HTMLPage):
    ...
    def iter_invest_rows(self, account):
        ...
        for row in self.doc.xpath('//th/div[contains(., "%s")]/ancestor::table//table/tbody/tr' % account.label):
            ...
            yield (
                row,
                (
                    self.doc.xpath('//div[contains(@id, "dv::s::%s")]' % id_repartition[0].rsplit(":", 1)[0])[0]
                    if id_repartition
                    else None
                ),
    ...
    def get_investment_form(self, form_param):
        return self.get_form(id="I0:P5:F", submit='.//input[@name = "%s"]' % form_param)
            ...
            if elem_repartition is not None:
                ...
                if not is_mirrored:
                    ...
                    if elem_diff is not None:
                        inv.diff_ratio = Eval(
                            lambda x: x / 100, CleanDecimal.French(Regexp(CleanText("."), r"([+-]?[\d\s]+[\d,]+)\s*%"))
                        )(elem_diff)
                else:
                    if elem_diff is not None:
    ...
    class fill_investment(ItemElement):
        ...
        def obj_asset_category(self):
            asset_category = Title(
                CleanText('//tr[th[text()="Classification AMF"]]/td', replace=[("LIBELLES EN EURO", "")])
            )(self)
...
class InvestmentDetailsPage(LoggedPage, HTMLPage):
    ...
    class fill_investment(ItemElement):
        ...
        obj_unitvalue = CleanDecimal.French('//tr[th[contains(text(), "Valeur de la part")]]//em', default=NotAvailable)
...
class OperationPage(LoggedPage, HTMLPage):
    ...
    class get_transactions(ListElement):
        ...
        class item(ItemElement):
            ...
            obj_date = Date(
                Regexp(CleanText('(//p[contains(@id, "smltitle")])[1]'), r"(\d{1,2}/\d{1,2}/\d+)"), dayfirst=True
            )
    ...
    class get_entreprise_transactions(TableElement):
        ...
        class item(ItemElement):
            ...
            obj_id = Regexp(CleanText('//span[contains(text(), "Détail du")]'), r"n° (\d+)")
            obj_amount = CleanDecimal.French(TableCell("amount"))
            obj_label = CleanText(TableCell("label"))
            obj_raw = Transaction.Raw(Field("label"))
            obj_date = Date(CleanText('(//th[text()="Date d\'effet"]//following::td)[1]'), dayfirst=True)
