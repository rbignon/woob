...
class TransactionsPage(LoggedPage, HTMLPage):
    ...
    def is_not_sorted(self, order="down"):
        ...
        return (
            len(
                self.doc.xpath(
                    '//table[@id="ContentTable_datas"]/thead/tr/th[1]//img[contains(@src, "tri_%s_on")]'
                    % translate[order]
                )
            )
            == 0
    ...
    def assert_first_page_or_go_there(self):
        if len(self.doc.xpath('//table[@id="tgDecorationFoot"]')) and not len(
            self.doc.xpath('//table[@id="tgDecorationFoot"]//b[contains(text(), "1")]')
        ):
