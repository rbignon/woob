...
class S2eBrowser(LoginBrowser, StatesMixin):
    ...
    etoile_gestion_characteristics = URL(
        r"https?://www.etoile-gestion.com/etg_fr_fr/ezjscore/.*", EtoileGestionCharacteristicsPage
    )
    ...
    new_bnp_investments = URL(
        r"https://personeo.epargne-retraite-entreprises.bnpparibas.com/portal/salarie-bnp", BNPInvestmentsPage
    )
    bnp_investment_details = URL(
        r"https://funds-api.bnpparibas.com/api/performances/(?P<id>\w+)", BNPInvestmentDetailsPage
    )
    # Esalia pages
    ...
    esalia_performance = URL(
        r"https://www.societegeneralegestion.fr/psSGGestionEntr/ezjscore/call(.*)_tab_2", EsaliaPerformancePage
    )
    # HSBC pages
            ...
            if inv._link:
                if self.bnp_investments.match(inv._link):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Server returned a Server Error when trying to fetch investment performances."
                        )
                        continue
                    ...
                    if not self.bnp_investments.match(self.url):
                        ...
                        self.logger.warning(
                            "Could not access BNP investments page, no investment details will be fetched."
                        )
                        continue
                    ...
                    if m:
                        ...
                        try:
                            ...
                            self.logger.warning(
                                "Could not connect to the BNP API, no investment details will be fetched."
                            )
                            continue
                        else:
                        ...
                        self.logger.warning(
                            "Could not fetch BNP investment ID in its label, no investment details will be fetched."
                        )
