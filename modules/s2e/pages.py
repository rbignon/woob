...
class LoginPage(HTMLPage):
    ...
    def check_error(self):
        if bool(
            self.doc.xpath(
                '//span[@class="operation-bloc-content-message-erreur-text"][contains(text(), "est incorrect")]'
            )
        ) or bool(
            self.doc.xpath(
                '//span[@class="operation-bloc-content-message-erreur-text"][contains(text(), "is incorrect")]'
            )
        ):
    ...
    def is_login_form_available(self):
        return HasElement('//form[@id="formulaireEnvoi"]')(self.doc) and HasElement(
            '//input[@id="identifiantClavierVirtuel"]'
        )(self.doc)
    ...
    def get_form_send_otp(self):
        ...
        receive_code_btn = bool(
            self.doc.xpath('//div[has-class("authentification-bloc-content-btn-bloc")][count(input)=1]')
        )
...
class CmCicInvestmentPage(LoggedPage, HTMLPage):
    ...
    def get_code(self):
        return CleanText(
            '//th[span[contains(text(), "Code valeur")]]/following-sibling::td//span', default=NotAvailable
        )(self.doc)
...
class AmundiPage(LoggedPage, HTMLPage, CodePage):
    ...
    def get_code(self):
        return Regexp(
            CleanText('//div[@class="amundi-fund-legend"]', default=NotAvailable), r"ISIN: (\w+)", default=NotAvailable
        )(self.doc)
...
class AMFSGPage(LoggedPage, HTMLPage, CodePage):
    ...
    def get_investment_performances(self):
        ...
        self.logger.warning(
            "This investment leads to AMFSGPage, please handle SRRI, asset_category and recommended_period."
        )
        ...
        durations = [
            CleanText(".")(el)
            for el in self.doc.xpath('//table[tr[th[contains(text(), "Performances glissantes")]]]//tr[2]//th')
        ]
        values = [
            CleanText(".")(el)
            for el in self.doc.xpath('//table[tr[th[contains(text(), "Performances glissantes")]]]//td')
        ]
        matches = dict(zip(durations, values))
...
class ItemInvestment(ItemElement):
    ...
    def parse(self, el):
        ...
        label = CleanText(label_block.xpath('.//div[contains(@style, "text-align")][1]'))(self)
        ...
        page = None
        link_id = Attr('.//a[contains(@title, "détail du fonds")]', "id", default=None)(self)
        ...
        if link_id and inv_id:
            ...
            if "hsbc.fr" in self.page.browser.BASEURL:
                ...
                url = Regexp(CleanText("//complete", default=""), r"openUrlFichesFonds\('([^']+)'", default="")(
                    page.doc
                )
                ...
                if m:
                    ...
                    for a_block in fund_links:
                        share_class = Regexp(Attr(".", "title"), r"- part (\w+) \(", default=NotAvailable)(a_block)
                        code = Regexp(Link("."), r"/fr/epargnants/fund-centre/(\w+)(?:\?.*)", default=NotAvailable)(
                            a_block
                        )
...
class MultiPage(HTMLPage):
    ...
    def get_multi(self):
        return [Attr(".", "value")(option) for option in self.doc.xpath('//select[@class="ComboEntreprise"]/option')]
...
class AccountsPage(LoggedPage, MultiPage):
    ...
    CONDITIONS = {
        "disponible": Pocket.CONDITION_AVAILABLE,
        "épargne": Pocket.CONDITION_AVAILABLE,
        "available": Pocket.CONDITION_AVAILABLE,
        "withdrawal": Pocket.CONDITION_RETIREMENT,
    ...
    class iter_accounts(TableElement):
        ...
        col_label = ["My schemes", "Mes dispositifs"]
        col_balance = [re.compile("Total"), re.compile("Montant")]
        ...
        class item(ItemElement):
            ...
            def condition(self):
                return self.xpath(
                    './/div[contains(@class, "mesavoirs-carre-couleur") and contains(@style, "background-color:#")]'
                )
            ...
            def obj_label(self):
                return Coalesce(CleanText(".//td[1]//a"), CleanText(".//td[1]/text()"), default=NotAvailable)(self)
    ...
    class iter_investment(TableElement):
        ...
        col_label = [re.compile("My investment"), re.compile("Mes supports")]
        col_valuation = [re.compile("Gross amount"), re.compile("Montant brut")]
        col_portfolio_share = ["Distribution", "Répartition"]
        col_diff = ["+ or - potential value", "+ ou - value potentielle"]
    ...
    class iter_pocket(TableElement):
        ...
        col_amount = [re.compile("Gross amount"), re.compile("Montant brut")]
        col_availability = ["Availability date", "Date de disponibilité"]
...
class HistoryPage(LoggedPage, MultiPage):
    ...
    class iter_history(TableElement):
        ...
        col_id = [re.compile("Ref"), re.compile("Réf")]
        col_date = [re.compile("Date"), re.compile("Creation date")]
        col_label = [re.compile("Transaction"), re.compile("Type")]
        ...
        class item(ItemElement):
            ...
            def obj_amount(self):
                ...
                employer_contrib = Base(
                    TableCell("net_employer_contribution_amount"), CleanDecimal.French(".//div", default=0)
                )(self)
            ...
            def match_account_transaction(self, el):
                ...
                if trid not in self.page.browser.cache["details"]:
                    ...
                    typeop = Regexp(Attr(TableCell("id")(self)[0].xpath("./a"), "onclick"), r"Operation.+?([A-Z_]+)")(
                        self
                    )
...
class SwissLifePage(HTMLPage, CodePage):
    ...
    def get_code(self):
        code = CleanText(
            '//span[contains(text(), "Code ISIN")]/following-sibling::span[@class="data"]', default=NotAvailable
        )(self.doc)
        if code == "n/a":
            return NotAvailable
        return code
...
class EtoileGestionPage(HTMLPage, CodePage):
    ...
    def get_code(self):
        ...
        characteristics_url = urljoin(
            self.url, Attr('//a[contains(text(), "Caractéristiques")]', "data-href", default=None)(self.doc)
        )
...
class EtoileGestionCharacteristicsPage(LoggedPage, PartialHTMLPage):
    ...
    def get_performance_history(self):
        ...
        if CleanDecimal.French('//tr[td[text()="Fonds"]]//td[position()=last()-2]', default=None)(self.doc):
            perfs[1] = Eval(
                lambda x: x / 100, CleanDecimal.French('//tr[td[text()="Fonds"]]//td[position()=last()-2]')
            )(self.doc)
        if CleanDecimal.French('//tr[td[text()="Fonds"]]//td[position()=last()-1]', default=None)(self.doc):
            perfs[3] = Eval(
                lambda x: x / 100, CleanDecimal.French('//tr[td[text()="Fonds"]]//td[position()=last()-1]')
            )(self.doc)
        if CleanDecimal.French('//tr[td[text()="Fonds"]]//td[position()=last()]', default=None)(self.doc):
            perfs[5] = Eval(lambda x: x / 100, CleanDecimal.French('//tr[td[text()="Fonds"]]//td[position()=last()]'))(
                self.doc
            )
        return perfs
...
class EsaliaPerformancePage(LoggedPage, HTMLPage):
    def get_performance_history(self):
        ...
        durations = [
            CleanText(".")(el) for el in self.doc.xpath('//div[contains(@class, "fpPerfglissanteclassique")]//th')
        ]
        values = [
            CleanText(".")(el)
            for el in self.doc.xpath('//div[contains(@class, "fpPerfglissanteclassique")]//tr[td[text()="Fonds"]]//td')
        ]
        matches = dict(zip(durations, values))
...
class AmundiDetailsPage(LoggedPage, HTMLPage):
    def get_recommended_period(self):
        return Title(
            CleanText(
                '//label[contains(text(), "Durée minimum de placement")]/following-sibling::span', default=NotAvailable
            )
        )(self.doc)

    def get_asset_category(self):
        return CleanText(
            '(//label[contains(text(), "Classe d\'actifs")])[1]/following-sibling::span', default=NotAvailable
        )(self.doc)
...
class ProfilePage(LoggedPage, MultiPage):
    ...
    class get_profile(ItemElement):
        ...
        obj__civilite = CleanText(
            '//div/span[contains(text(), "Civilité") or contains(text(), "Title")]/following-sibling::div/span'
        )
        obj_lastname = CleanText(
            '//div/span[contains(text(), "Nom") or contains(text(), "Name")]/following-sibling::div/span'
        )
        obj_firstname = CleanText(
            '//div/span[contains(text(), "Prénom") or contains(text(), "First name")]/following-sibling::div/span'
        )
        obj_name = Format("%s %s %s", obj__civilite, obj_firstname, obj_lastname)
        obj_address = CleanText(
            '//div/span[contains(text(), "Adresse postale") or contains(text(), "Postal address")]/following-sibling::div/div[2]'
        )
        obj_phone = CleanText(
            '//div/span[contains(text(), "Tél. portable") or contains(text(), "Mobile phone")]/following-sibling::div/span'
        )
...
class EServicePage(LoggedPage, HTMLPage):
    ...
    def show_more(self):
        ...
        try:
            ...
            button_el = form.el.xpath('.//input[matches(@value,"Tout afficher|Afficher tous")]')[0]
        except IndexError:
