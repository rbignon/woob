...
class SGPEPage(HTMLPage):
    def get_error(self):
        err = (
            ...
            or self.doc.xpath('//div[contains(text(), "Echec de connexion à l\'espace Entreprises")]')
            or self.doc.xpath('//div[contains(@class, "waitAuthJetonMsg")]')
        )
        if err:
...
class ChangePassPage(SGPEPage):
    def on_load(self):
        message = (
            ...
            or "Informations manquantes sur le site Société Générale"
        )
...
class MainPEPage(SGPEPage, PasswordPage):
    ...
    def login(self, login, password):
        ...
        self.browser.location(self.get_authentication_url(), data=authentication_data)
...
class MainPage(LoggedPage, SGPEPage):
    def get_market_accounts_link(self):
        ...
        if market_accounts_link:
            ...
            '//span[contains(text(), "Comptes-titres") and contains(@title, "pas habilité à utiliser ce service")]'
        ):
            return NotAvailable
...
class MarketAccountsDetailsPage(LoggedPage, HTMLPage):
    ...
    class iter_investment(TableElement):
        ...
        class item(ItemElement):
            ...
            def parse(self, el):
                messages = re.compile("En dépôt à l'étranger" + "|Dont indisponible" + "|PAS DE VALEURS")
