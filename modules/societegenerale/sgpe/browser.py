...
from .json_pages import (
    ...
    BalancesJsonPage,
    BankStatementPage,
    CardsInformation2Page,
    CardsInformationPage,
    CorpListPage,
    DeferredCardHistoryJsonPage,
...
class SGPEBrowser(SocieteGeneraleLogin):
    ...
    deferred_card = URL(
        r"/icd/gkb/data/getCartesPourPm-authsec.json\?b64e200_idPPouPM=(?P<card_id>.+)", DeferredCardJsonPage
    )
...
class SGEnterpriseBrowser(SGPEBrowser):
    ...
    # * Ent adapted URLs
    main_page = URL(r"https://entreprises.sg.fr", r"/sec/vk/gen_", MainPEPage)
...
class SGProfessionalBrowser(SGPEBrowser):
    ...
    add_recipient = URL(r"/ord-web/ord//ord-fragment-form-tiers.html\?cl_action=ajout&cl_idTiers=", AddRecipientPage)
    ...
    error_page = URL(r"https://static.sg.fr/pro/erreur.html", r"https://.*/pro/erreur.html", ErrorPage)

    # * Pro adapted URLs
    main_page = URL(r"https://professionnels.sg.fr", r"/sec/vk/gen_", MainPEPage)
                ...
                for loan in self.page.iter_loans():
                    ...
                    if self.page.get_loan_status() != "OK":
                        ...
                        if "Détail indisponible" in error_message:
                            ...
                            raise AssertionError(f"Unhandled error on pro_loan_details page: {error_message}")
    ...
    def init_transfer(self, account, recipient, transfer):
        ...
        if not self.page.is_date_valid(transfer.exec_date):
            raise TransferBankError(
                message="La date d'exécution du virement est invalide. Elle doit correspondre aux horaires et aux dates d'ouvertures d'agence."
            )
    ...
    def execute_transfer(self, transfer, **params):
        ...
        assert self.confirm_transfer.is_here(), "An error occurred, we should be on confirm transfer page."
