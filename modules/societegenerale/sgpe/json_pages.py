    ...
    def check_error(self):
        if self.doc["commun"]["statut"].lower() == "nok":
            ...
            if reason == "SYD-COMPTES-UNAUTHORIZED-ACCESS":
                ...
                raise ActionNeeded(
                    locale="fr-FR",
                    message="Votre banque requiert des informations complémentaires pour mettre à jour votre dossier client.",
                    action_type=ActionType.FILL_KYC,
                )
            elif reason in ("INSCRIP_OBL", "FIABILISATION_COORDONNEES"):
                raise ActionNeeded(
                    locale="fr-FR",
                    message="Veuillez vous rendre sur le site de votre banque pour completer vos informations.",
                    action_type=ActionType.FILL_KYC,
                )
            else:
...
class DeferredCardJsonPage(SGPEJsonPage):
    ...
    class iter_accounts(DictElement):
        ...
        class item(ItemElement):
            ...
            def condition(self):
                return not Dict("inactivityDate", default="")(self) and Dict(  # Is not always present in json
                    "currentOutstandingAmountDate"
                )(
                    self
                )  # avoid immediate debit cards

            obj_id = Coalesce(Dict("numeroCarte", default=NotAvailable), Dict("numeroCarteHash", default=NotAvailable))
            obj_number = Coalesce(
    ...
    def populate_balances(self, accounts):
        for account in accounts:
            ...
            account.balance = CleanDecimal(replace_dots=True).filter(
                acc_dict.get("soldeComptable", acc_dict.get("soldeInstantane"))
            )
            account.currency = Currency.get_currency(
                acc_dict.get("deviseSoldeComptable", acc_dict.get("deviseSoldeInstantane"))
            )
            account.coming = CleanDecimal(replace_dots=True, default=NotAvailable).filter(
                acc_dict.get("montantOperationJour")
            )
            yield account
    ...
    class iter_history(DictElement):
        ...
        class item(ItemElement):
            ...
            def obj_commission(self):
                if Regexp(
                    Field("label"), r" ([\d{1,3}\s?]*\d{1,3},\d{2}E COM [\d{1,3}\s?]*\d{1,3},\d{2}E)", default=""
                )(self):
            ...
            def obj_gross_amount(self):
                if not empty(Field("commission")(self)):
                    ...
                    return CleanDecimal.French(
                        Regexp(Field("label"), r" ([\d{1,3}\s?]*\d{1,3},\d{2})E COM", default=""), default=NotAvailable
                    )(self)

                return NotAvailable

            def obj_amount(self):
                return CleanDecimal(Dict("c", default=None), replace_dots=True, default=None)(self) or CleanDecimal(
                    Dict("d"), replace_dots=True
                )(self)
