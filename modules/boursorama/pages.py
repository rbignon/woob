...
class IbanPage(LoggedPage, HTMLPage):
    def get_iban(self):
        if self.doc.xpath('//div[has-class("alert")]/p[contains(text(), "Une erreur est survenue")]') or self.doc.xpath(
            '//div[has-class("alert")]/p[contains(text(), "Le compte est introuvable")]'
        ):
            return NotAvailable
        return CleanText(
...
class Transaction(FrenchTransaction):
    PATTERNS = [
        ...
        (
            re.compile(
                r"^(?P<text>.+)?((ACHAT|PAIEMENT)\s)?CARTE (?P<dd>\d{2})(?P<mm>\d{2})(?P<yy>\d{4}) (?P<text2>.*)"
            ),
            FrenchTransaction.TYPE_CARD,
        ),
        (
...
class AuthenticationPage(AuthenticationFormPage):
    def get_form_state(self):
        ...
        if payload:
            ...
            return Dict("challenges/0/parameters/formScreen/actions/check/api/params/formState")(json_payload)
    ...
    def has_skippable_2fa(self):
        return self.doc.xpath('//form[@name="form"]/div[contains(@data-strong-authentication-payload, "Ignorer")]')
...
class PasswordPage(LoginPage, AuthenticationFormPage):
    ...
    def get_error(self):
        return Coalesce(
            ...
            CleanText('//h2[contains(text(), "Erreur")]/following-sibling::div[contains(@class, "msg")]', default=""),
            default=None,
        )(self.doc)
    ...
    def get_document_cookie(self):
        ...
        cookie_value = Regexp(CleanText("//script"), rf'="{cookie_name}=(.*?);', default=None)(self.doc)
            ...
            class item(ItemElement):
                ...
                def obj_ownership(self):
                    ownership = Coalesce(
                        ...
                        MapIn(Lower(Field("label")), self.page.ACCOUNTS_OWNERSHIP, default=NotAvailable),
                        default=NotAvailable,
                    )(self)
...
class LoanPage(LoggedPage, HTMLPage):
    ...
    class fill_account(ItemElement):
        def obj_id(self):
            account_id = Coalesce(
                ...
                CleanText('//h3[has-class("c-product-title__sublabel")]', default=NotAvailable),
                default=NotAvailable,
            )(self)
    ...
    class get_loan(ItemElement):
        ...
        obj_id = Coalesce(
            ...
            CleanText('//h3[contains(@class, "c-product-title__sublabel")]', default=NotAvailable),
        )
        obj_number = obj_id
        ...
        obj_nb_payments_left = Eval(
            float_to_int, CleanDecimal.French('//p[contains(text(), "échéances restantes")]/span', default=NotAvailable)
        )
        obj_next_payment_amount = Coalesce(
            CleanDecimal.French('//p[contains(text(), "Montant de la prochaine échéance")]/span', default=NotAvailable),
            CleanDecimal.French(
        ...
        obj_total_amount = Coalesce(
            CleanDecimal.French('//p[matches(text(), "(Capital|Montant) emprunt")]/span', default=NotAvailable),
            CleanDecimal.French(
                '//div[matches(text(), "(Capital|Montant) (E|e)mprunt")]/following-sibling::div', default=NotAvailable
            ),
        )
        ...
        def obj_balance(self):
            balance = Coalesce(
                ...
                CleanDecimal.French('//p[contains(text(), "Capital restant dû")]//span', default=None),
            )(self)
            if balance > 0:
                balance *= -1
            return balance
...
class CalendarPage(LoggedPage, HTMLPage):
    def on_load(self):
        ...
        calendar_ics_url = urljoin(
            self.browser.BASEURL, CleanText('//a[contains(@href, "calendrier.ics")]/@href')(self.doc)
        )
...
class HistoryPage(LoggedPage, HasConfigurationPage):
    ...
    class fill_account(ItemElement):
        ...
        def obj_coming(self):
            ...
            return CleanDecimal.French('//li[h4[text()="Mouvements à venir"]]/h3', default=NotAvailable)(self)
        ...
        class item(ItemElement):
            ...
            def obj_coming(self):
                return (
                    ...
                    or len(self.xpath('.//div[has-class("list-operation-item__label-name--authorization")]'))
                ...
                if Env("is_card", default=False)(self):
                    if self.page.browser.deferred_card_calendar is None and self.page.browser.card_calendar_loaded:
                        ...
                        if has_coming:
                            date = Date(
                                CleanText(
                                    '//div[@data-operations-debit-informations=""]//span[contains(text(), "Montant total")]//span[1]'
                                ),
                                parse_func=parse_french_date,
                            )(self)
                            return date
    ...
    def get_csv_link(self):
        return Link('//a[@data-operations-export-button and not(has-class("hidden"))]', default=None)(self.doc)
...
class MarketPage(LoggedPage, HTMLPage):
        ...
        def obj_valuation_diff_ratio(self):
            ...
            v_diff_ratio = CleanDecimal.French(
                Coalesce(
                    ...
                    CleanText('//div[contains(text(), "+/- values en %")]/following-sibling::div'),
                    default=NotAvailable,
                ),
                default=NotAvailable,
            )(self)
    ...
    def get_balance(self, account_type):
        ...
        span_balance = CleanDecimal(
            '//li/span[contains(text(), "%s")]/following-sibling::span' % txt, replace_dots=True, default=None
        )(self.doc)
    ...
    class _iter_investment_gestion_profilee(Myiter_investment):
        ...
        class item(Myitem):
            ...
            obj_unitvalue = CleanDecimal.French(
                Base(TableCell("unitvalue"), CleanText("./span[not(@class)]")), default=NotAvailable
            )
    ...
    def get_liquidity(self):
        ...
        valuation = Coalesce(
            CleanDecimal.French('//li/*[contains(text(), "Solde Espèces")]/following-sibling::*', default=NotAvailable),
            CleanDecimal.French(
    ...
    class iter_market_orders(TableElement):
        ...
        class item(ItemElement):
            ...
            obj_stock_market = Base(TableCell("stock_market"), CleanText("./text()[2]"), default=NotAvailable)
            obj_payment_method = MapIn(
                CleanText(TableCell("stock_market")), MARKET_ORDER_PAYMENTS, MarketOrderPayment.UNKNOWN
            )
...
class AsvPage(MarketPage):
    ...
    def fetch_opening_date(self):
        return Date(
            CleanText('//div[contains(text(), "ouverture fiscale")]//strong'), dayfirst=True, default=NotAvailable
        )(self.doc)
...
class ErrorPage(HTMLPage):
    def on_load(self):
        error = Attr('//input[@required][@id="profile_lei_type_identifier"]', "data-message", default=None)(
            self.doc
...
class CardInformationPage(LoggedPage, HTMLPage):
    def get_card_number(self, card):
        ...
        card_number = Regexp(
            CleanText(f'//option[@value="{card._key}"]'), r"(\d{4}\*{8}\d{4})|^\d{4}\*{8}\d{4}$", default=NotAvailable
        )(self.doc)
    ...
    class iter_recipients(ListElement):
        ...
        class item(ItemElement):
            ...
            obj_bank_name = Regexp(
                CleanText('.//div[@class="transfer__account-name"]'), pattern=r"- ([^-]*)$", default=NotAvailable
            )

            def obj_label(self):
                label = Regexp(CleanText('.//div[@class="c-card-ghost__top-label"]'), pattern=r"^(.*?)(?: -[^-]*)?$")(
                    self
                )
...
class RecipientsPage(LoggedPage, HTMLPage):
    ...
    class iter_recipients(ListElement):
        ...
        obj_id = ""
        obj_label = CleanText(
            '//span[@class="o-list-inline__item ' + 'c-link-list-collapsable__label"]',
        )
        ...
        obj__sublabel = CleanText(
            '//span[@class="o-list-inline__item ' + 'c-link-list-collapsable__sub-label"]',
        )
    ...
    def submit_account(self, account_id):
        ...
        if "Vous ne possédez pas de compte éligible au virement" in no_account_msg:
            ...
            raise AssertionError(
                'Unhandled error message when trying to select an account for a new transfer: "%s"' % no_account_msg
            )
...
class NewTransferConfirm(LoggedPage, HTMLPage):
    ...
    class get_transfer(ItemElement):
        ...
        XPATH_TMPL = (
            '//form[@name="Confirm"]//tr[has-class("definition-list__row")][th[contains(text(),"%s")]]/td[1]/span[1]'
        )
...
class TransferCharacteristics(LoggedPage, HTMLPage):
    ...
    def submit_info(self, amount, label, exec_date):
        ...
        if datetime.date.today() == exec_date:
            assert (
                self.get_option(form.el.xpath('//select[@id="Characteristics_schedulingType"]')[0], "Ponctuel") == "1"
            )
            form["Characteristics[schedulingType]"] = "1"
        else:
    ...
    class get_transfer(ItemElement):
        ...
        def obj_exec_date(self):
            ...
            if type_ == "Ponctuel":
                ...
                return Date(
                    CleanText('//span[@id="transfer-date"]/span[@class="transfer__account-value"]'), dayfirst=True
                )(self)
...
class AddRecipientPage(TransferOtpPage):
    def on_load(self):
        ...
        if alert:
            if re.search(r"caract.ristiques habituelles", alert) and re.search(r"dans un d.lai de 72h", alert):
