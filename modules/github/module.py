...
STATUSES = {
    "open": Status("open", "open", Status.VALUE_NEW),
    "closed": Status("closed", "closed", Status.VALUE_RESOLVED),
}
...
class GithubModule(Module, CapBugTracker):
    ...
    def iter_issues(self, query):
        ...
        if (
            empty(query.assignee)
            and empty(query.author)
            and empty(query.status)
            and empty(query.title)
            and empty(query.tags)
        ):
    ...
    def _fetch_comments(self, issue):
        ...
        if not issue.history:
            issue.history = []
        issue.history += [
            self._make_comment(dcomment, issue.project)
            for dcomment in self.browser.iter_comments(project_id, issue_number)
        ]
    ...
    def _fetch_events(self, issue):
        ...
        if not issue.history:
            issue.history = []
        issue.history += [
            self._make_update(dcomment, issue.project)
            for dcomment in self.browser.iter_events(project_id, issue_number)
        ]
