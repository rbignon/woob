...
class FacebookBrowser(DomainBrowser):
    ...
    def login(self, username, password):
        self.location(
            "https://www.facebook.com/dialog/oauth?client_id=%s&redirect_uri=fbconnect://success&scope=email,user_birthday,user_friends,public_profile,user_photos,user_likes&response_type=token"
            % self.CLIENT_ID
        )
...
class HappnBrowser(DomainBrowser):
    ...
    def __init__(self, facebook, *args, **kwargs):
        ...
        r = self.request(
            "/api/users/%s/devices/" % self.my_id,
            data={
                "adid": "78462200ada313bb943b01800819a1d5",
    ...
    def get_contact(self, contact_id):
        data = self.request(
            "/api/users/%s?fields=birth_date,first_name,last_name,nickname,login,credits,referal,matching_preferences,notification_settings,unread_conversations,about,is_accepted,age,job,workplace,school,modification_date,profiles.mode(0).width(1000).height(1000).fields(url,width,height,mode),last_meet_position,my_relation,is_charmed,distance,gender"
            % contact_id
        )["data"]
        return data
    ...
    def get_thread(self, id):
        r = self.request(
            "/api/users/%s/conversations/%s?fields=id,messages.limit(100).fields(id,message,creation_date,sender.fields(id)),participants.fields(user.fields(birth_date,first_name,last_name,nickname,credits,referal,matching_preferences,notification_settings,unread_conversations,about,is_accepted,age,job,workplace,school,modification_date,profiles.mode(0).width(1000).height(1000).fields(url,width,height,mode),last_meet_position,my_relation,is_charmed,distance,gender))"
            % (self.my_id, id)
        )["data"]
        return r
    ...
    def find_users(self):
        return self.request(
            "/api/users/me/notifications?types=468&fields=id,is_pushed,lon,actions,creation_date,is_notified,lat,modification_date,notification_type,nb_times,notifier.fields(id,job,is_accepted,workplace,my_relation,distance,gender,my_conversation,is_charmed,nb_photos,last_name,first_name,age),notified.fields(is_accepted,is_charmed)"
        )["data"]
