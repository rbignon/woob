...
class HomePage(LoggedPage, HTMLPage):
    ...
    class get_subscriptions(ListElement):
        ...
        class item(ItemElement):
            ...
            def condition(self):
                return "href" in self.el.attrib and re.match(
                    ".*/FichePersonne/DetailPersonne[?]idDynamic=.*", self.el.attrib["href"]
                )
...
class MyAccountPage(LoggedPage, HTMLPage):
    ...
    class iter_documents(ListElement):
        ...
        class item(ItemElement):
            ...
            obj_id = Attr(".", "id")
            obj_url = Format(
                "https://www.espace-citoyens.net/%s/espace-citoyens/MonCompte/TelechargerPdfFacture?IdFactureUnique=%s",
                Env("city"),
                Field("id"),
            )
            obj_format = "pdf"
...
class BillingDetailPage(LoggedPage, HTMLPage):
    ...
    class get_document(ItemElement):
        ...
        obj_currency = "euro"
        obj_duedate = Date(
            CleanText('.//*[@id="spanDetailFacture_DatLimitePaiementFacture"]'), parse_func=parse_french_date
        )
