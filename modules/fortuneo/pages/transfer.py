...
class RecipientsPage(ActionNeededPage):
    ...
    def get_new_recipient(self, recipient):
        ...
        rcpt = Recipient()
        rcpt.label = Regexp(
            CleanText(recipient_xpath + '/li[contains(text(), "Nom du titulaire")]', replace=[(" ", "")]),
            r"(?<=Nomdutitulaire:)(\w+)",
        )(self.doc)
        rcpt.iban = Regexp(
            CleanText(recipient_xpath + '/li[contains(text(), "IBAN")]'), r"IBAN : ([A-Za-z]{2}[\dA-Za-z]+)"
        )(self.doc)
        rcpt.id = rcpt.iban
...
class ConfirmRecipientPage(LoggedPage, PartialHTMLPage):
    ...
    def is_code_expired(self):
        return self.doc.xpath(
            '//label[contains(text(), "Le code sécurité est expiré. Veuillez saisir le nouveau code reçu")]'
        )
    ...
    def get_error(self):
        return CleanText('//form//p[@class="container error"]//label[@class="error"]')(self.doc)
...
class RegisterTransferPage(LoggedPage, HTMLPage):
    ...
    def get_recipient_transfer_id(self, recipient):
        for account in self.doc.xpath('//select[@name="compteACrediter"]/option[not(@selected)]'):
            ...
            if recipient.id == recipient_transfer_id or recipient.id in CleanText(".", replace=[(" ", "")])(account):
    ...
    def fill_transfer_form_and_get_used_recipient_id(self, account, recipient, amount, label, exec_date):
        ...
        form["libelleCompteADebiter"] = CleanText('//select[@name="compteADebiter"]/option[@value="%s"]' % account.id)(
            self.doc
        )
...
class ValidateTransferPage(LoggedPage, HTMLPage):
    def on_load(self):
        errors_msg = (
            ...
            CleanText('//div[@id="error" and @class="erreur_texte"]/p[contains(text(), "n\'est pas autorisé")]')(
                self.doc
            ),
    ...
    def check_transfer_data(self, transfer_data):
        for t_data in transfer_data:
            assert t_data in transfer_data[t_data], "{} not found in transfer summary {}".format(
                t_data, transfer_data[t_data]
            )
    ...
    def handle_response(self, account, recipient, amount, label, exec_date, used_recipient_id):
        ...
        transfer_data = {
            account.id: CleanText(summary_xpath + '/li[contains(text(), "Compte à débiter")]')(self.doc),
            ...
            recipient._recipient_name: CleanText(summary_xpath + '/li[contains(text(), "Nom du bénéficiaire")]')(
                self.doc
            ),
        ...
        transfer.exec_date = Date(
            Regexp(CleanText(summary_xpath + '/li[contains(text(), "Date de virement")]'), r"(\d+/\d+/\d+)"),
            dayfirst=True,
        )(self.doc)
...
class ConfirmTransferPage(LoggedPage, HTMLPage):
    ...
    def confirm_transfer(self):
        confirm_transfer_url = (
            "/fr/prive/mes-comptes/compte-courant/realiser-operations/effectuer-virement/confirmer-saisie-virement.jsp"
        )
    ...
    def transfer_confirmation(self, transfer):
        if self.doc.xpath(
            '//div[@class="confirmation_virement"]/h2[contains(text(), "virement a bien été enregistrée")]'
        ):
            return transfer
