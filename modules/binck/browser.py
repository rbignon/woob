...
class BinckBrowser(LoginBrowser):
    ...
    home_page = URL(r"/$", r"/Home/Index", HomePage)
    ...
    history = URL(r"/TransactionsOverview/GetTransactions", r"/TransactionsOverview/FilteredOverview", HistoryPage)
    questions = URL(
        r"/FDL_Complex_FR_Compte", r"/FDL_NonComplex_FR_Compte", r"FsmaMandatoryQuestionnairesOverview", QuestionPage
    )
    change_pass = URL(r"/ChangePassword/Index", r"/EditSetting/GetSetting\?code=MutationPassword", ChangePassPage)
    ...
    def iter_old_accounts(self):
        ...
        for a in self.page.iter_accounts():
            ...
            data = {"grouping": "SecurityCategory"}
            try:
                a._invpage = self.investment.go(data=data, headers=token) if self.page.is_investment() else None
            except HTTPNotFound:
