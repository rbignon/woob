...
class BPBrowser(LoginBrowser, StatesMixin):
    ...
    redirect_page = URL(
        r".*voscomptes/identification/identification.ea.*", r".*voscomptes/synthese/3-synthese.ea", RedirectPage
    )

    auth_page = URL(
        r"voscomptes/canalXHTML/securite/gestionAuthentificationForte/init-gestionAuthentificationForte.ea", TwoFAPage
    )
    ...
    polling_MPIN_page = URL("/voscomptes/canalXHTML/securisation/mpin/validerOperation-securisationMPIN.ea", Polling2FA)
    ...
    par_accounts_checking = URL(
        "/voscomptes/canalXHTML/comptesCommun/synthese_ccp/afficheSyntheseCCP-synthese_ccp.ea", AccountList
    )
    par_accounts_savings_and_invests = URL(
        "/voscomptes/canalXHTML/comptesCommun/synthese_ep/afficheSyntheseEP-synthese_ep.ea", AccountList
    )
    ...
    personal_loan_routage_url = URL(
        "/voscomptes/canalXHTML/sso/espaceDigitalLBPF/routage-espaceDigitalLBPF.ea", PersonalLoanRoutagePage
    )
    revolving_redirection = URL(
        "/voscomptes/canalXHTML/pret/encours/detaillerCreditRenouvelableListe-encoursPrets.ea", RevolvingPage
    )
    revolving_details = URL(
        "/voscomptes/canalXHTML/pret/creditRenouvelable/init-consulterCreditRenouvelable.ea", RevolvingPage
    )
    accounts_rib = URL(
    ...
    market_check = URL(
        r"/voscomptes/canalXHTML/bourse/aiguillage/lancerBourseEnLigne-connexionBourseEnLigne.ea", MarketCheckPage
    )
    ...
    cards_json_detail = URL(
        r"/ws_q44/api/v1/cartes\?numCompte=(?P<account_id>\w+)&typeListe=consultation", CardsJsonDetails
    )
    transfer_choose = URL(
        r"/voscomptes/canalXHTML/virement/mpiaiguillage/init-saisieComptes.ea", TransferChooseAccounts
    )
    ...
    certicode_plus_submit_device = URL(
        r"/voscomptes/canalXHTML/securisation/mpin/demandeCreation-securisationMPIN.ea", CerticodePlusSubmitDevicePage
    )
    rcpt_code = URL(
    ...
    post_login_page = URL(r"https://voscomptesenligne.labanquepostale.fr/wsost/OstBrokerWeb/auth", PostLoginPage)

    pre_mandate = URL(r"/voscomptes/canalXHTML/sso/commun/init-integration.ea\?partenaire=procapital", PreMandate)
    pre_mandate_bis = URL(
        r"https://www.gestion-sous-mandat.labanquepostale-gestionprivee.fr/lbpgp/secure/main.html", PreMandateBis
    )
    ...
    subscription = URL(
        "/voscomptes/canalXHTML/relevePdf/relevePdf_historique/reinitialiser-historiqueRelevesPDF.ea", SubscriptionPage
    )
    subscription_search = URL(
        r"/voscomptes/canalXHTML/relevePdf/relevePdf_historique/form-historiqueRelevesPDF\.ea", SubscriptionPage
    )
    download_page = URL(
        ...
        if self.auth_page.is_here():
            ...
            if auth_method == "cer+":
                ...
                if self.par_accounts_checking.is_here():
                    ...
                    self.logger.warning(
                        "The bp module should now trigger an authentication challenge on your cell phone, but this part has never been tested (2024-05-02)"
                    )
                ...
                if self.no_terminal.is_here() and self.page.has_no_terminal():
                    raise ActionNeeded(
                        locale="fr-FR",
                        message="Veuillez associer votre téléphone à votre compte bancaire pour réaliser l'authentification forte",
                ...
                self.location(
                    "/voscomptes/canalXHTML/securite/gestionAuthentificationForte/authenticateCerticode-gestionAuthentificationForte.ea"
                )
                ...
                raise ActionNeeded(
                    locale="fr-FR",
                    message="Veuillez activer votre service gratuit d'authentification forte sur votre site bancaire.",
                    action_type=ActionType.ENABLE_MFA,
                )
    ...
    def fill_account_ownership(self, account):
        ...
        pattern = re.compile(r"(m|mr|me|mme|mlle|mle|ml)\.? (.*)\bou ?(m|mr|me|mme|mlle|mle|ml)?\b(.*)", re.IGNORECASE)
        if pattern.search(account._account_holder):
            account.ownership = AccountOwnership.CO_OWNER
        elif all(n in account._account_holder for n in owner_name.lower().split(" ")):
    ...
    def iter_cards(self, account):
        ...
        if self.cards_list.is_here():
            ...
            self.cards_json_detail.go(account_id=account.id, headers={"DISFE-CCX-Code-Appelant": "0004"})
    ...
    def get_history(self, account):
        ...
        if account.type == Account.TYPE_CARD and not account.url:
            ...
            self.cards_json_detail.go(account_id=account.parent.id, headers={"DISFE-CCX-Code-Appelant": "0004"})
    ...
    def get_coming(self, account):
        ...
        if account.type == Account.TYPE_CARD:
            return [tr for tr in self.iter_card_transactions(account) if tr.coming]
    ...
    def end_with_polling(self, obj):
        ...
        self.location(
            self.absurl("/voscomptes/canalXHTML/securisation/mpin/operationSucces-securisationMPIN.ea", base=True)
        )
        return obj
    ...
    def init_new_recipient(self, recipient, is_bp_account=False, **params):
        ...
        raise AddRecipientStep(
            self.build_recipient(recipient), Value("code", label="Veuillez saisir le code reçu par SMS")
        )
...
class BProBrowser(BPBrowser):
    ...
    redirect_after_vk = URL(r".*voscomptes/identification/identification.ea.*", RedirectAfterVKPage)
    switch_q5c = URL(r"/ws_q47/voscomptes/switchQ5C/redirectSyntheseQ5C-switchQ5C.ea", SwitchQ5CPage)
    detect_2fa = URL(
    ...
    # bank
    pro_accounts_list = URL(r"/ws_q5c/api/pmo/recupererSyntheseComptes", ProAccountsList)
    rib_choice = URL(r"/ws_q47/voscomptes/switchQ5C/rib-switchQ5C.ea", DownloadRib)
    rib = URL(r"/ws_q47/voscomptes/rib/preparerRIB-rib.ea", RibPage)
    pro_history = URL(
        r"/ws_q5c/api/pmo/comptes/(?P<account_id>\w+)/operations\?typeOperations=IMPUTEES", ProAccountHistory
    )

    # market
    useless2 = URL(
        r".*/voscomptes/bourseenligne/lancementBourseEnLigne-bourseenligne.ea\?numCompte=(?P<account>\d+)", UselessPage
    )
    ...
    redirect_page = URL(r".*voscomptes/synthese/3-synthese.ea", RedirectPage)
    ...
    def go_linebourse(self, account):
        self.location(
            "/ws_q47/voscomptes/bourseenligne/lancementBourseEnLigne-bourseenligne.ea", params={"numCompte": account.id}
    ...
    def get_history(self, account):
        ...
        try:
            ...
            )  # seems to fetch by default max nb of transactions without pagination (last 3 months)
        except ServerError as e:
