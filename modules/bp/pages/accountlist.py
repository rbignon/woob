...
class item_account_generic(ItemElement):
        ...
        return Coalesce(
            ...
            and not any(
                (
                    ...
                    self.el.xpath(
                        './/div[contains(@class, "amount")]//span[contains(text(), "Remboursé intégralement")]'
                    ),
    ...
    def obj_coming(self):
        if Field("type")(self) == Account.TYPE_CHECKING and Field("balance")(self) != 0:
            ...
            coming_op_link = Coalesce(
                Link('//a[contains(text(), "Opérations") and contains(text()[2], "à venir")]', default=None),
            ...
            if CleanText('.//dt[contains(., "Débit différé à débiter")]')(self):
                has_coming = True
                coming += CleanDecimal(
                    './/dt[contains(., "Débit différé à débiter")]/following-sibling::dd[1]', replace_dots=True
                )(self)
    ...
    obj__has_deferred_history = Link(
        './/div[contains(@class, "additional-data")]//a[contains(@href, init-mouvementsCarteDD)]', default=False
    )

    def obj__has_transfer(self):
        return bool(self.xpath('//ul[@class="cartridge-links"]//a/span[contains(text(), "Virement")]'))
...
class AccountList(LoggedPage, MyHTMLPage):
    ...
    def get_error(self):
        return CleanText('//div[contains(text(), "momentanément indisponible.")]')(self.doc) and not CleanText(
            '//div[text()="Montant emprunté"]'
        )(self.doc)
    ...
    class get_personal_loan(ItemElement):
        ...
        obj_nb_payments_left = CleanDecimal(
            CleanText('//div[div[contains(text(), "restant à rembourser")]]/div[2]'), default=NotAvailable
        )
        obj_next_payment_date = Date(
            CleanText('//div[div[contains(text(), "prochaine échéance")]]/div[2]'),
            dayfirst=True,
            default=NotAvailable,
        )
    ...
    class iter_loans(TableElement):
        ...
        col_maturity_date = re.compile("Date dernière")
        ...
        class item_loans(ItemElement):
            ...
            def obj_id(self):
                ...
                if CleanText('//select[@id="numOffrePretSelection"]/option[@selected="selected"]')(self):
                    return Regexp(
                        CleanText('//select[@id="numOffrePretSelection"]/option[@selected="selected"]'), r"(\d+)"
                    )(self)
            ...
            def obj_balance(self):
                if CleanText(TableCell("balance"))(self) != "Remboursé intégralement":
            ...
            def obj_subscription_date(self):
                ...
                if "souscrite le" in CleanText(xpath)(self):
                    return MyDate(Regexp(CleanText(xpath), r" (\d{2}/\d{2}/\d{4})", default=NotAvailable))(self)

                return NotAvailable

            obj_next_payment_amount = CleanDecimal(
                TableCell("next_payment_amount"), replace_dots=True, default=NotAvailable
            )

            def obj_maturity_date(self):
                if Field("subscription_date")(self):
                    async_page = Async("details").loaded_page(self)
                    return MyDate(CleanText('//div[@class="bloc Tmargin"]/dl[2]/dd[4]'))(async_page.doc)
            ...
            def obj_last_payment_date(self):
                ...
                async_page = Async("details").loaded_page(self)
                return MyDate(CleanText('//div[@class="bloc Tmargin"]/dl[1]/dd[2]'), default=NotAvailable)(
                    async_page.doc
                )
...
class Advisor(LoggedPage, MyHTMLPage):
    ...
    class get_advisor(ItemElement):
        ...
        def parse(self, el):
            ...
            agency_phone = CleanText(
                '//span/a[contains(@href, "rendezVous")]', replace=[(" ", "")], default=NotAvailable
            )(self) or CleanText('//div[has-class("lbp-numero")]/span', replace=[(" ", "")], default=NotAvailable)(self)

            advisor_phone = Regexp(
                CleanText('//div[h3[contains(text(), "conseil")]]//span[2]', replace=[(" ", "")]), r"(\d+)", default=""
            )(self)
            ...
            name = CleanText('//div[h3[contains(text(), "conseil")]]//span[1]', default=None)(self) or CleanText(
                '//div[@class="lbp-font-accueil"]/div[2]/div[1]/span[1]', default=None
            )(self)
            if name:
                self.env["name"] = name
...
class RevolvingPage(LoggedPage, HTMLPage):
    ...
    class fill_revolving(ItemElement):
        obj_total_amount = CleanDecimal.French(
            '//span[text()="Montant maximum autorisé"]/following-sibling::span/text()', default=NotAvailable
        )
        obj_available_amount = CleanDecimal.French(
            '//span[text()="Montant disponible"]/following-sibling::span/text()', default=NotAvailable
        )
        obj_next_payment_date = Date(
            CleanText('//span[text()="Date du prélèvement"]/following-sibling::span/text()'), dayfirst=True
        )
        ...
        def obj_insurance_label(self):
            label = CleanText(
                '//span[contains(text(), "Assurance")]/following-sibling::span/text()', default="Aucune Assurance"
            )(self)
        ...
        def obj_insurance_amount(self):
            ...
            if empty(Field("insurance_label")(self)) or empty(
                CleanText('//span[contains(text(), "Dont assurance au titre")]', default=None)(self)
            ):
                return NotAvailable
            amount = 0
