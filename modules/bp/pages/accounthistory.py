...
class Transaction(FrenchTransaction):
    PATTERNS = [
        ...
        (
            re.compile(
                r"^CARTE X\d{4} (?P<dd>\d{2})/(?P<mm>\d{2})/(?P<yy>\d{2,4}) A \d{2}H\d{2} (?P<text>(?P<category>RETRAIT DAB) .*)"
            ),
        ...
        (
            re.compile(
                r"^(?P<category>CREDIT CARTE BANCAIRE) (?P<text>.*) (?P<dd>\d{2})\.(?P<mm>\d{2})\.(?P<yy>\d{2,4}) .*"
            ),
            FrenchTransaction.TYPE_CARD,
        ),
...
class AccountHistory(LoggedPage, MyHTMLPage):
    ...
    def generate_card_summary(self):
        ...
        tr.date = tr.rdate = Date(Regexp(text, r"[Montant imputé le|cours prélevé au] (\d+/\d+/\d+)"), dayfirst=True)(
            self.doc
        )
    ...
    def has_transactions(self):
        return not CleanText("""//table[contains(@id, 'mouvements')]//tr[contains(., "as d'opération")]""")(self.doc)
...
class TemporaryPage(LoggedPage, MyHTMLPage):
    def get_next_link(self):
        return self.absurl(Regexp(CleanText("//script"), r'location.replace\([\'"](.*)[\'"]\)')(self.doc))
    ...
    class iter_cards(DictElement):
        ...
        class item(ItemElement):
            ...
            obj_coming = CleanDecimal.US(
                Dict("encoursCarte/listeOuverte/0/montantEncours", default=None), default=NotAvailable
            )
            obj_currency = Currency(Dict("encoursCarte/listeOuverte/0/deviseEncours", default=""), default=NotAvailable)
            obj_owner_type = AccountOwnerType.PRIVATE
            obj__account_holder = NotAvailable
