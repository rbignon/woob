...
class CmsoLoginBrowser(TwoFactorBrowser):
    ...
    login = URL(r"/securityapi/checkuser", r"/auth/checkuser", LoginPage)
    logout = URL(r"/securityapi/revoke", r"https://.*/auth/errorauthn", LogoutPage)
    ...
    def init_login(self):
        ...
        try:
            response = self.authorization_codegen_uri.go(
                data=data, params={"session_id": self.login_session_id}, headers=headers
            )
    ...
    def handle_sms(self):
        ...
        headers = self.get_tpp_headers(data)
        self.authorization_codegen_uri.go(params={"session_id": self.login_session_id}, data=data, headers=headers)
...
class CmsoParBrowser(CmsoLoginBrowser):
    ...
    available_int_recipients = URL(
        r"/transfersfedesapi/api/credited-accounts/(?P<ciphered_contract_number>.*)", AllowedRecipientsPage
    )
    ...
    def _go_market_history(self, action):
        try:
            url_before_market_history = json.loads(self.market.go(json={"place": "SITUATION_PORTEFEUILLE"}).text)[
                "urlSSO"
            ]
        except KeyError:
    ...
    def iter_recipients(self, account):
        ...
        if not account:
            self.logger.info(
                "Either account cannot make transfers or the link between emitters and the account could not be made."
            )
            return
