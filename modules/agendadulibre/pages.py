...
class EventPage(HTMLPage):
    ...
    class get_event(ItemElement):
        ...
        obj_summary = Format(
            "%s %s", CleanText('//meta[@name="geo:placename"]/@content'), CleanText('//meta[@name="DC:title"]/@content')
        )
...
class EventListPage(HTMLPage):
    ...
    class list_events(ListElement):
        ...
        class item(ItemElement):
            ...
            def condition(self):
                return len(XPath(".")(self.el)) > 0 and ("current-month" in XPath("./ancestor::td/@class")(self.el)[0])

            obj_id = Format(
                "%s#%s", CleanText('./ancestor::td/div[@class="day_number"]'), Regexp(Link("./a"), "/events/(.*)")
            )
