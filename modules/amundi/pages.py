    ...
    class iter_history(DictElement):
        ...
        class item(ItemElement):
            ...
            def condition(self):
                ...
                if CleanText(Dict("statut"))(self) != "CPTA" or "Arbitrage" in Field("label")(self):
                    return False
...
class EEProductInvestmentPage(LoggedPage, HTMLPage):
    ...
    class fill_investment(ItemElement):
        ...
        obj_recommended_period = CleanText(
            '//span[contains(text(), "Durée minimum")]/following-sibling::span[@class="valeur"][1]'
        )
...
class BNPInvestmentPage(LoggedPage, HTMLPage):
    def get_fund_id(self):
        return Regexp(
            CleanText('//script[contains(text(), "GLB_ProductId")]'), r'GLB_ProductId = "(\w+)', default=None
        )(self.doc)
...
class EcofiInvestmentPage(LoggedPage, HTMLPage):
    ...
    class fill_investment(ItemElement):
        ...
        obj_recommended_period = Regexp(
            CleanText(Attr('//img[contains(@src, "/Horizon/")]', "src", default=NotAvailable), replace=[("_", " ")]),
            r"\/Horizon (.*)\.png",
        )
        obj_asset_category = CleanText(
...
class OlisnetInvestmentPage(LoggedPage, HTMLPage):
    ...
    def get_performance(self):
        perf = CleanDecimal.SI(
            Regexp(CleanText("."), r"Portefeuille : (-?\d+\.?\d*?)%", default=NotAvailable), default=NotAvailable
        )(self.doc)
