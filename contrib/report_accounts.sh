...
woob bank -q -f table list > ${TMPSTORAGE}/account_list

ACCOUNTS=`awk '/@/ {print $2}' ${TMPSTORAGE}/account_list`

for i in $ACCOUNTS
do
	woob bank -q -f table history "$i" > ${TMPSTORAGE}/account_history_${i}
	woob bank -q -f table coming "$i"  > ${TMPSTORAGE}/account_coming_${i}
done

printf "Bank accounts report, generated by woob bank\n" > ${TMPSTORAGE}/account_mail
