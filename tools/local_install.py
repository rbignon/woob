...
if len(sys.argv) < 2:
    ...
    print()
    print("Error: Please provide a destination, " "for example ‘%s/bin’" % os.getenv("HOME"), file=sys.stderr)
    sys.exit(1)
else:
...
if local_modules:
    sourceslist = os.path.join(
        os.environ.get("XDG_CONFIG_HOME", os.path.join(os.path.expanduser("~"), ".config")), "woob", "sources.list"
    )
    ...
    if not os.path.exists(sourceslist):
        with open(sourceslist, "w") as f:
            f.write("file://" + os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir, "modules")))
