...
class AppWoobathon(ReplApplication):
    APPNAME = 'woob-athon'
    VERSION = '2.1'
    COPYRIGHT = 'Copyright(C) 2011-YEAR Romain Bignon'
    DESCRIPTION = 'Console application to participate to a Woobathon.'
    SHORT_DESCRIPTION = "participate in a Woobathon"
    CAPS = CapContent
    SYNOPSIS =  'Usage: %prog [-dqv] [-b backends] [-cnfs] woobathon\n'
    SYNOPSIS += '       %prog [--help] [--version]'

    radios = []

    def __init__(self, *args, **kwargs):
        super(AppWoobathon, self).__init__(*args, **kwargs)
    ...
    def do_join(self, line):
        ...
        if self.event.is_closed():
            print("Woobathon is closed.", file=self.stderr)
            return 1
