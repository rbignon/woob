...
# start with:
# set PYTHONPATH=D:\Dropbox\Projets\money
# python d:\Dropbox\Projets\money\scripts\money -N
...
class HistoryThread(Thread):
    def __init__(self, money, account):
        ...
        self.disabled = money.config.get(account, 'disabled', default=False)
        self.date_min = money.config.get(account, 'date_min', default='')
        self.last_date = money.config.get(account, 'last_date', default='')

    @property
    def label(self):
        if self.account in self.money.labels:
            return self.money.labels[self.account]
        else:
            return self.money.config.get(self.account, 'label', default='')
    ...
    def run(self):
        ...
        if self.money.options.force:
            from_date = self.date_min
        else:
            from_date = self.last_date

        if from_date >= now:
            self.money.print(Style.BRIGHT + "%s (%s): Last import date is %s, no need to import again..." % (
        ...
        if content == '':
            ...
            with numMutex:
                self.money.importIndex = self.money.importIndex + 1
                index = self.money.importIndex
            self.money.logger.error("(%i/%i) %s (%s): %saborting after %i retries.%s" % (
                index, len(self.money.threads),
                self.account,
                self.label,
        ...
        for line in input:
            ...
            if re.match(r'^</STMTTRN>', line):
                ...
                if os.path.exists(moduleHandler):
                    self.money.logger.info("Calling backend handler %s..." % moduleHandler)
        ...
        ofxfile = os.path.join(self.money.getDownloadsPath(), fname + ".ofx")
        with open(ofxfile, "w") as ofx_file:
            ofx_file.write(re.sub(r'\r\n', r'\n', ofxcontent.encode(sys.stdout.encoding)))

        with numMutex:
            self.money.write(stderrcontent)
            self.money.importIndex = self.money.importIndex + 1
            index = self.money.importIndex
        if not (self.money.options.noimport or nbTransactions == 0):
            self.money.backupIfNeeded()
        with printMutex:
            if self.money.options.noimport or nbTransactions == 0:
                if nbTransactions == 0:
        ...
        if not self.money.options.noimport:
            if nbTransactions > 0:
                subprocess.check_call('"%s" %s' % (
                    os.path.join(self.money.getMoneyPath(), "mnyimprt.exe"),
                    ofxfile))
            self.last_date = now
