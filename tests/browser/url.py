...
def test_ishere_klass_none(my_browser):
    ...
    with pytest.raises(
        AssertionError,
        match=r"You can use this method only if there is a " + "Page class handler.",
    ):
...
def test_with_headers():
    ...
    assert other_url._timeout == 5.0
    assert third_url._headers == {"X-Oh-No": "wow"}  # non cumulative!
    assert third_url._timeout == 5.0
    assert fourth_url._headers is None
    assert fourth_url._timeout == 5.0


def test_with_timeout():
    url = URL(r"mypath", timeout=5.0)
    second_url = url.without_timeout()
    third_url = url.with_timeout(6.0)

    assert url._timeout == 5.0
    assert second_url._timeout is None
    assert third_url._timeout == 6.0
